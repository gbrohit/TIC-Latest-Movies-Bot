...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('ᴛᴜᴛᴏʀɪᴀʟ ᴠɪᴅᴇᴏ', url=f'https://t.me/TheInsomniacsClub/461')
        ],[
            InlineKeyboardButton('⇋ ʙᴀᴄᴋ ⇋', callback_data='start')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f'sᴇᴀᴄʜɪɴɢ ғᴏʀ {search}', url=f"https://t.me/TheInsomniacsClub")]]))
            search = search.lower()
            find = search.split(" ")
            search = ""
